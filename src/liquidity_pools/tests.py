...
class PoolHistoryLoaderServiceTest(TransactionTestCase):
    def setUp(self):
        self.chain, _ = Chain.objects.update_or_create(
            id=42161,
            name='Arbitrum One',
            slug='arbitrum',
            rpc_urls=[
        ...
        self.w3 = W3Service(chain_id=self.chain.id)
        self.pool_history_loader_service = PoolHistoryLoaderService(
            w3=self.w3,
    ...
    def test_load(self):
        ...
        response = self.pool_history_loader_service.load(
            from_block=block_number - 128,
