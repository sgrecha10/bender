...
from market_data.choices import IntervalCodename, INTERVAL_DURATION
...
class TestHelperMixin:
    ...
    def create_interval(self,
                        codename: str = None,
                        value: str = None,
                        minutes_count: int = None) -> Interval:
        codename = codename if codename else IntervalCodename.MINUTE_1.name
        result, _ = Interval.objects.get_or_create(
            codename=codename,
            defaults={
                value: value if value else getattr(IntervalCodename, codename).value,
                minutes_count: minutes_count if minutes_count else INTERVAL_DURATION[codename],
            }
        )
        return result
