...
class StrategyBackendBase:
    strategy_codename = None
    ...
    def close_all_position(self, idx: datetime, price: Decimal = None):
        ...
        if self.has_long_position:
            self._make_sell(state=StrategyResult.State.UNKNOWN, price=price, deal_time=idx)
            self.has_long_position = False

        elif self.has_short_position:
            self._make_buy(state=StrategyResult.State.UNKNOWN, price=price, deal_time=idx)
            self.has_short_position = False

    def _check_init_params(self) -> Optional[str]:
        """ Проверяем, что все необходимые данные инициализированы

        Если что то не так, райзить ексепшн
        :return
        """
        return
    ...
    def _set_stop_loss(self, sd: Decimal, buy: Decimal = None, sell: Decimal = None):
    ...
    def _set_take_profit(self, sd: Decimal, buy: Decimal = None, sell: Decimal = None):
    ...
    def _check_stoploss(self, deal_time: datetime, price: Decimal):
        if self.has_long_position and price <= self.stop_loss:
            _, is_ok = self._make_sell(
        ...
        elif self.has_short_position and price >= self.stop_loss:
            _, is_ok = self._make_buy(
                state=StrategyResult.State.LOSS,
                price=price,
                deal_time=deal_time,
            )
    ...
    def _check_takeprofit(self, deal_time: datetime, price: Decimal):
        if self.has_long_position and price >= self.take_profit:
            _, is_ok = self._make_sell(
        ...
        elif self.has_short_position and price <= self.take_profit:
            _, is_ok = self._make_buy(
                state=StrategyResult.State.PROFIT,
                price=price,
                deal_time=deal_time,
...
class StrategyFirstBackend(StrategyBackendBase):
    """ Алгоритм
    ...
    def run_step(self, deal_time: datetime, price: Decimal):
        ...
        if self.has_long_position:
            ...
            if price >= previous_ma_value > previous_close_price:
                ...
                real_price, is_ok = self._make_buy(
                ...
                if is_ok:
                    ...
                    self._set_stop_loss(sd=previous_sd_value, buy=real_price)
                    self._set_take_profit(sd=previous_sd_value, buy=real_price)
            ...
            if price <= previous_ma_value < previous_close_price:
                ...
                real_price, is_ok = self._make_sell(
                    state=StrategyResult.State.OPEN,
                    price=previous_ma_value,
                    deal_time=deal_time,
                )
                if is_ok:
                    ...
                    self._set_stop_loss(sd=previous_sd_value, sell=real_price)
                    self._set_take_profit(sd=previous_sd_value, sell=real_price)
...
class AnnaStrategyBackend(StrategyBackendBase):
    strategy_codename = Strategy.Codename.STRATEGY_ANNA
    moving_average_codename_1 = 'MA_10'
    moving_average_codename_2 = 'MA_20'
    standard_deviation_codename = 'SD_30'
    ...
    def run_step(self, deal_time: datetime, price: Decimal):
        ...
        # Проверяем стопы и тейкпрофиты
        self._check_stoploss(deal_time=deal_time, price=price)
        self._check_takeprofit(deal_time=deal_time, price=price)

        # Находим позицию индекса текущей свечи
        index_position = self.kline_df.index.get_loc(idx)
        if index_position < 2:
            return
        last_index = self.kline_df.index[index_position - 1]
        previous_index = self.kline_df.index[index_position - 2]

        # Получаем значения МА за две предыдущих свечи
        last_ma_1_value = self.moving_average_1.get_value_by_index(
            index=last_index,
            source_df=self.source_df,
        )
        previous_ma_1_value = self.moving_average_1.get_value_by_index(
            index=previous_index,
            source_df=self.source_df,
        )
        last_ma_2_value = self.moving_average_2.get_value_by_index(
            index=last_index,
            source_df=self.source_df,
        )
        previous_ma_2_value = self.moving_average_2.get_value_by_index(
            index=previous_index,
            source_df=self.source_df,
        )

        # Получаем sd для выставления стопов/тейкпрофита
        sd_value = self.standard_deviation.get_value_by_index(
            index=last_index,
            source_df=self.source_df,
        )

        # print(deal_time)
        # Проверяем, было ли пересечение МА
        # ma_2 сверху вниз, sell
        if previous_ma_1_value < previous_ma_2_value and last_ma_1_value > last_ma_2_value:

            # Проверяем тестовые настройки стратегии
            if self.strategy.direction_deals == Strategy.Direction.ONLY_BUY:
                return

            # Открываем позицию если нет открытой позиции
            if not (self.has_long_position or self.has_short_position):
                real_price, is_ok = self._make_sell(
                    state=StrategyResult.State.OPEN,
                    price=price,
                    deal_time=deal_time,
                ...
                if is_ok:
                    self.has_short_position = True
                    self._set_stop_loss(sd=sd_value, sell=real_price)
                    self._set_take_profit(sd=sd_value, sell=real_price)

        # ma_2 снизу вверх, buy
        elif previous_ma_1_value > previous_ma_2_value and last_ma_1_value < last_ma_2_value:
            ...
            if not (self.has_long_position or self.has_short_position):
                ...
                if is_ok:
                    self.has_long_position = True
                    self._set_stop_loss(sd=sd_value, buy=real_price)
                    self._set_take_profit(sd=sd_value, buy=real_price)
